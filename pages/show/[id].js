...
export default function Data({ detail }) {
  ...
  return (
    <div>
      <Head>
        <title>Disney | {detail.title || detail.name}</title>
      </Head>
      <Header />
      {!session ? (
        <Hero />
        ...
        <section className="relative z-50">
          <div className="relative min-h-[calc(100vh-72px)] opacity-80">
            <Image
              src={`${BASE_URL_IMAGE}${
...
// 2 - arrumar fetch dos shows - check
// 6 - fechar trailer ao clicar fora - check
